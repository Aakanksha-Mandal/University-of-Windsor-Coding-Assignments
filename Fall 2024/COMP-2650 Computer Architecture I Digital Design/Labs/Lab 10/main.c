...
void to_MAXTERM(int truth_table[][INPUT_VARIABLE_COUNT + OUTPUT_VARIABLE_COUNT]) {
    for (int j = 0; j < OUTPUT_VARIABLE_COUNT; j++) {
        printf("output variable F%d = ∏M(", j + 1);
        int first_maxterm = 1; // Flag to handle comma placement
        for (int i = 0; i < TRUTH_TABLE_ROW_COUNT; i++) {
            // Check if the output is 0 for this row and output variable
            if (truth_table[i][INPUT_VARIABLE_COUNT + j] == 0) {
                if (!first_maxterm) {
                    printf(", ");
                }
                first_maxterm = 0;
                printf("%d", i); // Print the maxterm index
            }
        }
        printf(") = ");
        int is_first_term = 1;
        for (int i = 0; i < TRUTH_TABLE_ROW_COUNT; i++) {
            // Check if the output is 0 for this row and output variable
            if (truth_table[i][INPUT_VARIABLE_COUNT + j] == 0) {
                // Print no '+' before the first maxterm
                if (!is_first_term) {
                    printf("");
                }
                is_first_term = 0;
                printf("("); // Start of the maxterm
                // Generate the maxterm for this row
                for (int k = 0; k < INPUT_VARIABLE_COUNT; k++) {
                    if (truth_table[i][k] == 0) {
                        printf("%c", 'Z' - k); // If variable is 0, use it normally
                    } else {
                        printf("%c'", 'Z' - k); // If variable is 1, negate it
                    }
                    if (k < INPUT_VARIABLE_COUNT - 1) {
                        printf("+"); // Separate the variables with '+'
                    }
                }
                printf(")"); // End of the maxterm
            }
        }
        printf("\n");
    }
...
int main(void) {
    ...
    while (1) {
        ...
        if (command == 1) {
            ...
            //printing the boolean function for F1 in the form of a sum of maxterms (canonical PoS)
